...
from td_sequential_scanner import TdSequentialScanner
from td_sequential_store import TdSequentialStore, format_scan_progress
...
def run_scan_job(job_id: str, *, trade_date: str | None = None, trigger_type: str = "manual") -> None:
    ...
    try:
        ...
        scanner = TdSequentialScanner(DB_PATH, get_settings=hist.get_settings)
        ...
        logger.exception("td sequential scan job failed")
        td_store.update_scan_job(
            job_id,
            status="failed",
            finished_at=datetime.now(CST).isoformat(),
...
def enqueue_td_scan(
    *,
    ...
    with _lock:
        if _running:
            active = TdSequentialStore(DB_PATH).get_active_scan_job()
            if active:
                return {"job_id": active["job_id"], "status": active["status"], "reused": True}
            raise RuntimeError("神奇九转扫描任务运行中")
        _running = True

    td_store = TdSequentialStore(DB_PATH)
    td = _resolve_trade_date(trade_date)
    job_id = td_store.create_scan_job(td, trigger_type)
...
def get_scan_status(job_id: str | None = None) -> dict[str, Any]:
    td_store = TdSequentialStore(DB_PATH)
    if job_id:
        job = td_store.get_scan_job(job_id)
    else:
        job = td_store.get_active_scan_job() or td_store.get_latest_scan_job()
    if not job:
        return {"active": False}
    return {
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="神奇九转扫描任务")
    ap.add_argument("--scheduled", action="store_true")
    ap.add_argument("--enqueue", action="store_true")
    ns = ap.parse_args()
    if ns.scheduled:
        td = _resolve_trade_date(None)
        store = TdSequentialStore(DB_PATH)
        job_id = store.create_scan_job(td, "scheduled")
        run_scan_job(job_id, trade_date=td, trigger_type="scheduled")
    elif ns.enqueue:
        print(json.dumps(enqueue_td_scan(trigger_type="manual"), ensure_ascii=False))
    else:
        ap.print_help()
